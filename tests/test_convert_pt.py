...
def _convert(target, variant):
    ...
    try:
        # AddHCATMixin calls commons.hcat.load_ec_mapping, which is a different function
        # from the commons.ec one test_convert.py patches, so pt.csv is pinned here too.
        with patch("fiboa_cli.datasets.commons.hcat.load_ec_mapping", side_effect=_load_ec):
...
def test_ids_are_unique_across_the_edition(converted, variant):
    ...
    # The invariant is on *distinct* ids, not row count. The base converter explodes
    # multipart geometries after post_migrate (base.py:406), so one field can legitimately
    # become several rows sharing an id -- 161 ids over 364 rows in the real 2017. What may
    # never happen is two source features claiming the same land occupation, which is what
    # 2018's repeated member would cause. The fixtures happen to be single-part, so rows
    # and distinct ids coincide here; the assertion is written so that a future multipart
    # fixture does not fail it for the wrong reason.
    assert df["id"].nunique() == FIXTURE_ROWS[variant], (
        f"{variant}: {df['id'].nunique()} distinct ids, expected {FIXTURE_ROWS[variant]}"
    )
...
def test_unresolved_question_mark_raises():
    ...
    with raises(AssertionError, match=r"still contain '\?'"):
...
def test_an_undeclared_member_overlap_raises(tmp_path):
    """Only 2018's Norte_S is known to repeat another member.

    Any other overlap means two members cover the same fields, and the edition would be
    inflated by exactly that many rows with no other symptom.
    """
    from fiboa_cli.datasets import pt

    with patch.dict(pt.OVERLAPPING_MEMBERS, {"2018": {}}, clear=False):
        with raises(AssertionError, match="already published by"):
            _convert(tmp_path / "x.parquet", "2018")
