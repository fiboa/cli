...
- REST converters: a variant may name its own service (es_ib keeps its yearly editions in a second one), every metadata request is retried, and a joined layer's table prefixes are stripped for every converter
