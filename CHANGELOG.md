...
- Adopt vecorel-cli 0.2.20, whose `convert()` chooses the first declared variant when `--variant` is not given. The converters that repeated that default in their own `get_urls()` (DE-BW, DE-HE, DE-ST, ES, ES-AN, ES-CB, ES-CM, ES-EX, ES-GA, ES-IB, ES-VC) drop it, LV gets it (it ran only with an explicit `--variant`), and ES-PV's default moves from 2024 to its newest variant, 2025, like every other converter
